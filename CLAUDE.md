...
AI chatbot application (Chat SDK v3.1.0) built with Next.js 16, React 19, and Vercel AI SDK. Supports multiple LLM providers via Vercel AI Gateway with real-time streaming, document/code editing (artifacts), and multi-modal input.
...
Dynamic route: `/chat/[id]` for existing conversations. New chats start at `/`.
...
- **Auth:** NextAuth v5 with Credentials provider + guest users. JWT sessions with user ID and type (`guest` | `regular`).
- **Chat streaming:** POST to `/api/chat` → AI SDK `streamText()` with data stream writer → client consumes via `DataStreamHandler` component
- **State:** Server Components for data fetching, SWR for client-side, Context API for streaming state (`DataStreamProvider`)
- **Messages:** `Message_v2` schema with JSON `parts` field supporting text, code, images, sheets. Legacy `Message` table exists but is deprecated.

### Database Schema (Drizzle ORM)

Core tables: `User`, `Chat`, `Message_v2` (parts-based messages), `Document` (versioned by composite PK `id + createdAt`), `Suggestion`, `Vote_v2`, `Stream`.

Config reads from `.env.local`. Schema at `lib/db/schema.ts`, migrations at `lib/db/migrations/`.

### AI Integration

Models defined in `lib/ai/models.ts`. Default model: `google/gemini-2.5-flash-lite`. All models accessed via `@ai-sdk/gateway`. Providers include Anthropic, OpenAI, Google, xAI, plus reasoning variants.

AI tools are in `lib/ai/tools/` — each tool returns structured results consumed by the artifact system.
...
- `import type` / `export type` for type-only imports/exports
- No `var` — use `const` or `let`
- Strict equality (`===` / `!==`)
- No unused variables, imports, or parameters
- No `@ts-ignore`
- Next.js: use `<Image>` not `<img>`, no `<head>` elements

**Excluded from linting:** `node_modules`, `.next`, `ai-sdk`, `components/ui`, `lib/utils.ts`, `hooks/use-mobile.ts`

**Disabled rules** (intentionally relaxed from Ultracite defaults): `noExplicitAny`, `noConsole`, `noMagicNumbers`, `noNestedTernary`, `noExcessiveCognitiveComplexity`, `noBarrelFile`, `noSvgWithoutTitle`
