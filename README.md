...
Built on top of [Chat SDK](https://chat-sdk.dev) v3.1.0, so you also get a full chat interface that can talk directly to your OpenClaw agent — including inline rendering of images generated by skills like nano-banana-pro.

[Watch the demo](https://youtu.be/zNM0L0SO1K8?si=dROdIr9ohhZFDW34)
...
When the agent generates an image (e.g. via the nano-banana-pro skill), the `MEDIA: /path/to/file.png` marker in the response is automatically detected, stripped from the text, and rendered as an inline `<img>`. Images are proxied through `/api/openclaw/media` which fetches them from a static file server on the gateway VPS, so no credentials are ever exposed to the browser.
...
When an assistant message contains a `MEDIA:` marker, the chat components extract the file path, strip the marker from the rendered text, and display an `<img>` tag pointing at `/api/openclaw/media?path=...`. That route proxies the request to a static file server (`/ocmedia/`) running on the gateway VPS, returning the binary with the correct content-type and an immutable cache header.
...
└── api/
    ├── openclaw/          # 14 dashboard API routes + media proxy
